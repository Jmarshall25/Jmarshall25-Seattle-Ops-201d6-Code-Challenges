...
$events = Get-EventLog -LogName System -After (Get-Date).AddDays(-1) | Out-File "$env:USERPROFILE\Desktop\last_24.txt"

# Output all “error” type events from the System event log to a file on your desktop named errors.txt.

$events = Get-EventLog -LogName System -EntryType Error | Out-File "$env:USERPROFILE\Desktop\errors.txt"

# Print to the screen all events with ID of 16 from the System event log.

$events = Get-EventLog -LogName System -Newest 16
...
$sources = $events | Select-Object -ExpandProperty Source -Unique
$sources